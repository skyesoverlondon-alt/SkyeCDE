  ...
  function render(){
    ...
    document.querySelector('#debate-grid').innerHTML = data.debates.map((d, i) => `
      <article class="debate-card" data-searchable>
        ...
        <div class="card-actions"><button class="btn btn-soft" data-vote="debate-${i}">${state.votes[`debate-${i}`] ? 'Voted' : 'Cast vote'}</button><button class="btn btn-soft">Open evidence</button></div>
      </article>
    `).join('');
  }
  render();
