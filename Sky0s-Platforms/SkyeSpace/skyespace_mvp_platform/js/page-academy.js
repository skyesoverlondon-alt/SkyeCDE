  ...
  function render(){
    ...
    document.querySelector('#course-grid').innerHTML = data.courses.map((c, i) => `
      <article class="course-card" data-searchable>
        ...
        <h3>${c.title}</h3>
        <p>${c.seats}</p>
        <div class="card-actions"><button class="btn btn-soft" data-enroll="course-${i}">${state.enrollments[`course-${i}`] ? 'Enrolled' : 'Enroll'}</button><button class="btn btn-soft">View syllabus</button></div>
      </article>
    `).join('');
  }
  render();
