...
const CryptoVault = (() => {
  ...
  // --- Attachments (encrypted in browser) ---
  async function encryptFileForRecipient(publicKeyPem, file){
    ...
    const ptBuf = await file.arrayBuffer();
    const { ivB64, ctB64 } = await aesGcmEncrypt(aesKey, ptBuf);

    return {
      filename: file.name || "attachment",
      mime_type: file.type || "application/octet-stream",
      size_bytes: ptBuf.byteLength,
      encrypted_key_b64: bufToB64(encKey),
      iv_b64: ivB64,
      ciphertext_b64: ctB64
    };
  ...
  return {
    ...
    decryptAttachmentWithPrivateKey
  };
})()