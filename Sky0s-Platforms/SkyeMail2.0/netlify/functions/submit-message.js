...
exports.handler = async (event) => {
  try{
    ...
    const ipH = hashIp(ip);
    const bucketIp = `ip:${ipH}:submit`;
    const bucketFrom = `from:${from_email.toLowerCase()}:submit`;
    ...
    // modest per-sender throttle too (prevents single email rotating IPs)
    const fr = await query(
      `select count(*)::int as c
       from rate_events
       where bucket=$1 and created_at > now() - interval '1 hour'`,
      [bucketFrom]
    );
    if(fr.rows[0].c >= 40){
      const err = new Error("Rate limit exceeded: too many messages from this sender (1 hour).");
      err.statusCode = 429;
      throw err;
    }
    ...
    await query(`insert into rate_events(bucket) values($1)`, [bucketFrom]);
    ...
    await recordAudit(event, null, 'MESSAGE_SUBMIT', 'user', user.id, { handle: user.handle, from_email, has_attachments: Array.isArray(body.attachments) && body.attachments.length>0 }, user.org_id);
