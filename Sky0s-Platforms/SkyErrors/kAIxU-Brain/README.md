...
This brain now ships with a built-in error reporting lane for Cloudflare Workers:

- Ingest: `POST /v1/errors/event`
- List:   `GET /v1/errors/events`
- Get:    `GET /v1/errors/events/:id`
- Admin:  `GET /v1/admin/errors/events` (requires `SKYE_ERRORS_ADMIN_SECRET`)

Docs: `docs/skye-errors.md`  
Schema: `skye-errors/schema.sql`  
SDK: `skye-errors/sdk/skye-errors-sdk.mjs`
...
## Hardened Auth (Level 500)

- **Authority mode (recommended):** set `KAIXU_SERVICE_SECRET` and at least one of `KAIXU_TOKEN_VERIFY_URL` or `KAIXU_NETLIFY_URLS` so the brain validates tokens against your central Gate token authority.
- **No header-based discovery:** the brain will *not* trust client headers (Origin/Referer) to discover verification endpoints.
- **Hashed token fallback:** if you must use env tokens, prefer `KAIXU_APP_TOKENS_SHA256` (comma-separated SHA-256 hex digests) over raw `KAIXU_APP_TOKENS`.
- **Per-token model allowlists:** if the authority returns `allowedModels` / `allowedEmbedModels`, requests are enforced at runtime.
