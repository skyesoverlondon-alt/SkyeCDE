import { authLogin, authLogout, authMe, listBookingMessages, listBookingUpdates, listBookings, postBookingMessage, postBookingUpdate, filesToPayload } from './hub-api.js';
...
$("signUpBtn").addEventListener("click", ()=>{
  toast(authStatus, "Caregiver accounts are provisioned by approval. Contact ops if your access is missing.", "bad");
});
...
$("postUpdateBtn").addEventListener("click", async ()=>{
  ...
  try{
    ...
    await postBookingUpdate(currentBookingId, note, photos);
    $("updateNote").value = "";
    $("updatePhotos").value = "";
...
async function syncAuthUI(){
  ...
  if(currentUser.role !== 'caregiver'){
    toast(authStatus, "This account is not approved as a caregiver yet. Contact ops.", "bad");
    await authLogout().catch(() => {});
    currentUser = null;
    authCard.style.display = "block";
    mainCard.style.display = "none";
