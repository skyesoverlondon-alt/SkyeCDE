...
const COOKIE_NAME = 'noblesoul_session';
...
async function verifyPassword(password, stored) {
  ...
  const actual = await pbkdf2Hash(password, parts[3]);
  const a = Buffer.from(actual);
  const b = Buffer.from(parts[4]);
...
async function createSession(userId) {
  ...
  await query('INSERT INTO hub_sessions (token, user_id, expires_at) VALUES ($1, $2, $3)', [token, userId, expires.toISOString()]);
...
async function requireRole(event, roles) {
  ...
  if (!roles.includes(user.role)) return { ok: false, response: json(403, { ok: false, error: 'forbidden' }) };
  return { ok: true, user };
}
...
function parseAdminEmails() {
  return String(process.env.ADMIN_EMAILS || '').split(',').map((value) => normalizeEmail(value)).filter(Boolean);
}
