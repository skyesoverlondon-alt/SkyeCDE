...
function draftKey(){ return "noble_soul_cohost_draft_v1"; }
...
async function uploadFiles(submissionId){
  ...
  for(const [inputName, bucket] of fileInputs){
    ...
    for(const file of Array.from(inp.files)){
      ...
      const path = `cohost_intake/${submissionId}/${bucket}/${Date.now()}_${safeName}`;
...
form.addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    await setDoc(doc(db, "cohost_intake", submissionId), payload);
