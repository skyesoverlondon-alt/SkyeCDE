  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const updates = await query(
        'SELECT id, note, photos, created_by_user_id, created_by_role, created_at FROM stay_updates WHERE stay_id = $1 ORDER BY created_at DESC',
        [stayId]
      );
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (!stayId || !note) return json(400, { ok: false, error: 'missing_fields' });

      const stay = await loadStay(stayId);
      if (!stay) return json(404, { ok: false, error: 'stay_not_found' });
      if (!(user.role === 'admin' || stay.cohost_uid === user.id)) return json(403, { ok: false, error: 'forbidden' });

      await query(
        `
          INSERT INTO stay_updates (id, stay_id, note, photos, created_by_user_id, created_by_role, created_at)
          VALUES (gen_random_uuid()::text, $1, $2, $3::jsonb, $4, $5, NOW())
        `,
        [stayId, note, JSON.stringify(photos), user.id, user.role]
      );
