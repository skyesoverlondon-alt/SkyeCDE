...
$('adminSignup')?.addEventListener('click', async () => {
  try {
    const data = await authSignup($('adminEmail').value.trim(), $('adminPass').value, 'SOL Mobile Detailing Admin', 'admin');
...
$('exportBtn').addEventListener('click', () => {
  ...
  link.download = 'sol-mobile-detailing-intake-export.json';
  link.click();
  URL.revokeObjectURL(link.href);
});

$('approveDetailerBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    const data = await approveDetailer(selectedSubmissionId);
    setStatus('approveStatus', data.tempPassword ? `Approved. User ID: ${data.uid}. Temporary password: ${data.tempPassword}` : `Approved. User ID: ${data.uid}. Existing account kept.`, 'ok');
    await refresh();
  } catch (error) {
...
$('createJobBtn').addEventListener('click', async () => {
  try {
    setStatus('createJobStatus', 'Creating…');
    const data = await createJob({
      clientUid: $('bkClientUid').value.trim(),
      clientEmail: $('bkClientEmail').value.trim(),
      clientName: $('bkClientName').value.trim(),
      detailerUid: $('bkDetailerUid').value.trim(),
      detailerEmail: $('bkDetailerEmail').value.trim(),
      detailerName: $('bkDetailerName').value.trim(),
      address: $('bkAddress').value.trim(),
      zip: $('bkZip').value.trim(),
      preferredDate: $('bkPreferredDate').value,
      timeWindow: $('bkTimeWindow').value.trim(),
      vehicleCount: $('bkVehicleCount').value.trim(),
      vehicleType: $('bkVehicleType').value.trim(),
      serviceLevel: $('bkServiceLevel').value,
      interior: $('bkInterior').value,
      heavySoil: $('bkHeavySoil').value,
      requestType: $('bkRequestType').value,
      plan: $('bkPlan').value.trim(),
      addons: {
        engine: $('bkEngine').value,
        headlights: $('bkHeadlights').value,
        odor: $('bkOdor').value,
      },
      notes: $('bkNotes').value.trim(),
    });
    setStatus('createJobStatus', `Job created: ${data.jobId}`, 'ok');
  } catch (error) {
    setStatus('createJobStatus', 'Create failed: ' + (error.message || error), 'bad');
  }
});
