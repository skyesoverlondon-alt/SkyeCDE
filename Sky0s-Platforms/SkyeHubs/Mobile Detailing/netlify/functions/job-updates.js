...
exports.handler = async (event) => {
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const updates = await query('SELECT id, note, photos, created_by_user_id, created_by_role, created_at FROM job_updates WHERE job_id = $1 ORDER BY created_at DESC', [jobId]);
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (!jobId || !note) return json(400, { ok: false, error: 'missing_fields' });
      const job = await loadJob(jobId);
      if (!job) return json(404, { ok: false, error: 'job_not_found' });
      if (!(user.role === 'admin' || job.detailer_uid === user.id)) return json(403, { ok: false, error: 'forbidden' });
      await query(
        'INSERT INTO job_updates (id, job_id, note, photos, created_by_user_id, created_by_role, created_at) VALUES (gen_random_uuid()::text, $1, $2, $3::jsonb, $4, $5, NOW())',
        [jobId, note, JSON.stringify(photos), user.id, user.role]
      );
