...
exports.handler = async (event) => {
  try{
    ...
    const ref = db.collection("detailer_intake").doc(submissionId);
    ...
    const city = (data.home?.city || "").trim(); // reuse field
    const zip = (data.home?.zip || "").trim();
    const maxVehicles = parseInt(data.capacity?.maxPets || "0", 10) || 2; // reuse intake key but interpret as vehicles
    const tools = data.experience?.medsExperience || "Unknown"; // reuse key but interpret as tools list
    const waterAccess = String(data.home?.fencedYard || "").toLowerCase().includes("yes"); // reuse key
    ...
    await db.collection("detailers").doc(uid).set({
      uid,
      email,
      name,
      city,
      zip,
      maxVehiclesPerSlot: maxVehicles || 2,
      waterAccess: waterAccess ? "yes" : "unknown",
      tools: tools,
      status: "active",
