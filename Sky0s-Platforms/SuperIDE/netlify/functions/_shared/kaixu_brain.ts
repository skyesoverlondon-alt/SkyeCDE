...
function normalizeKaixuGatewayEndpoint(raw: string): string {
  ...
  // Redirect any legacy skyesol.netlify.app URLs to 0megaSkyeGate (skyesol is decommissioned)
...
export async function callKaixuBrainWithFailover({
  ...
  const endpoint = normalizeKaixuGatewayEndpoint(opt("KAIXU_GATEWAY_ENDPOINT", "https://0megaskyegate.skyesoverlondon.workers.dev"));
